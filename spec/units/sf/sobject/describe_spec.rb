RSpec.describe 'SfCli::Sf::Sobject' do
  ...
  describe '#describe' do
    it "returns the schema infromation of an Object" do
      allow(sf).to receive(:exec).with('sobject', :describe, flags: {:"target-org" => nil, sobject: 'Account'}, switches: {}, redirection: :null_stderr).and_return(exec_output)
    ...
    it 'can retrieve a object information in a paticular org, not default one' do
        allow(sf).to receive(:exec).with('sobject', :describe, flags: {:"target-org" => :dev, sobject: 'Account'}, switches: {}, redirection: :null_stderr).and_return(exec_output)
